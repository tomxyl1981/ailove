...
public class EmailLoginActivity extends AppCompatActivity {
    ...
    private static final String SEND_CODE_URL = "https://jiehun.mynatapp.cc/user/send-code";
    private static final String LOGIN_URL = "https://jiehun.mynatapp.cc/user/login";
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    String receivedCode = jsonResponse.optString("code", "");
                    
                    Log.d(TAG, "success: " + success + ", code: " + receivedCode);
                    
                    if (success && !receivedCode.isEmpty() && !receivedCode.equals("-1")) {
    ...
    private void login() {
        ...
        // 调用后端登录接口
        loginToServer(email, inputCode);
    }

    private void loginToServer(String email, String code) {
        JSONObject json = new JSONObject();
        try {
            json.put("email", email);
            json.put("code", code);
        ...
        httpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> showError("网络请求失败: " + e.getMessage()));
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    runOnUiThread(() -> showError("登录失败: " + e.getMessage()));
                }
            }
        });
    }
